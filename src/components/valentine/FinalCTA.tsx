...
export function FinalCTA() {
  const scrollToPricing = () => {
    const pricingSection = document.querySelector('[class*="EXCLUSIVE TICKETS"]')?.closest('section') ||
      document.querySelector('section:has(.grid.md\\:grid-cols-2.gap-8)');
    if (pricingSection) {
      pricingSection.scrollIntoView({ behavior: 'smooth' });
    } else {
      // Fallback: scroll to pricing by finding the section with pricing cards
      const sections = document.querySelectorAll('section');
      sections.forEach(section => {
        if (section.textContent?.includes('Choose Your') && section.textContent?.includes('Experience')) {
      ...
      <div className="max-w-4xl mx-auto relative z-10 text-center">
        ...
        >
          ...
          <div className="space-y-2 text-gray-500 text-sm">
            ...
            <p className="text-[#B76E79]">Saturday, February 14, 2026 • 6 PM</p>
            <p>Rainbow Towers VIP Lounge, Harare • Limited to 100 Guests</p>
          </div>
        </motion.div>
      </div>
    </section>
